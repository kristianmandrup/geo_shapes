module Geo
  class Radius < Point
    class Rectangular < Geo::Radius
    attr_accessor :vector_distance

    def initialize center_point, vector_distance
      super center_point
      vector_distance = Geo::Vector.create_from vector_distance
      raise ArgumentError, "#{self.class} radius distance must be a Geo::Vector with lat and long distance" if !vector_distance.kind_of? Geo::Vector
      @vector_distance = vector_distance
    end  

    def multiply arg
      rect = Geo::Radius::Rectangular.new center, vector_distance.clone
      rect.multiply! arg      
    end
    ...
    def create_from *args
      ...
      when 1
        arg = args.first
        if arg.kind_of? Geo::Shape::Rectangle
          rectangle = arg 
    ...
    def radius_from_factors factors
      if factors.all_same?
        ...
        rectangle = Geo::Radius::Rectangular.create_from self
